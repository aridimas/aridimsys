    ...
    <>
      ...
      >
        <div className="container-xl flex items-center justify-between">
          <Link href="/" className="nav-text font-medium hover:text-foreground-secondary transition-colors">
            Portfolio
          </Link>
            ...
            <nav className="flex space-x-16">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="nav-text transition-colors duration-500 text-foreground-secondary hover:text-foreground"
                >
                  {item.name}
                </Link>
              ))}
